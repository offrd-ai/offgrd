...
(function (global) {
  ...
  function hydrateView(prior, side) {
    adopt(prior);
    var fromJ = activeEvents(side);
    if (fromJ.length) return fromJ;
    var EU = global.OFFGRD_EMPTY_UNKNOWN;
    if (EU && EU.isUnknownEmpty(fromJ, { confirmedEmpty: false }) && prior && prior.length) {
      return prior;
    }
    return prior && prior.length ? prior : fromJ;
  }
  ...
  function census(opts) {
    ...
    var rows = (gameId ? eventsForGame(gameId) : activeEvents(side)).filter(function (r) {
      if (gameId && gameIsCleared(gameId) && !(opts.includeCleared)) return false;
      if (side && r.side && r.side !== side) return false;
      return isLedgerEvent(r);
    });
    if (gameId && gameIsCleared(gameId) && !opts.includeCleared) rows = [];
    var saved = rows.length;
    var snaps = log ? log.length : saved;
    var syn = syncedSet(side || (opts.side || "offense"));
    var synced = 0;
    rows.forEach(function (r) {
    ...
    return {
      ...
      tone: tone,
      label: label,
      undoUntil: gameId ? clearUndoUntil(gameId) : null,
    };
  }
