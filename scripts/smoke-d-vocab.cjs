...
/* Call of record first; perspective-spoken label (not bare Run/Pass) as fallback. */
ok(/hero = look \|\| speakPlayLabel\(on\)/.test(DC), "Last-play hero is the D call of record, not just Run/Pass");
