...
const fridaySit = { dn: 4, db: "GOAL", namedCall: "OMAHA", zone: "REDZONE", hash: "ANY" };
check(
  "leftover Friday sit with empty events is immune",
...
(async function () {
  ...
  check(
    "same opponent on reopen does not mint",
    S.shouldRotateForOpponent(priorSess, "Parkway South") === false
  );
  check(
    "inProgress with no opp name does not mint",
    S.shouldRotateForOpponent({ gameId: "frag", inProgress: true }, "Parkway Central", fridayEvents) === false
  );
  check(
    "Live fallback does not mint a new gameId",
    S.shouldRotateForOpponent({ gameId: "g1", opp: "Live", inProgress: true }, "Parkway Central") === false
  );
  ...
  check(
    "no-name leftover inProgress does not rotate",
    S.shouldRotateForOpponent({ gameId: "anon", inProgress: true }, "Parkway Central", fridayEvents) === false
  );
