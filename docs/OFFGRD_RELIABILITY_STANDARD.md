...
- Game identity: `gameId` assigned once at session creation, stored on
  every row. No code path may reassign it. Week 2 deletes restamp as a
  concept; a stale session is simply *listed* under its real date.
...
**Pinned v361 (Mon 2026-09-07 night, Matt override).** Chip must read
**v361**. Claude verifies in-browser: census line on both callers,
journal rows in IndexedDB on every tap, Clear as a journal row with
Undo, view rebuilds from the journal after the derived store is wiped
by hand.
- **Tuesday soak (old iPad, Matt).** Scenario 1: airplane, 60 snaps
  ...
  Either fail → iPads stay on v360; keep the export files.
- **Wednesday practice** is the gate for Friday: both callers, DC
  ...
  lands in Files.
- **Thursday freeze. Friday Sep 11 on v361.**
