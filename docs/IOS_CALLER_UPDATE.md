...
The caller chip **v361** (top bar, next to Snap N) is the only proof the refresh took.
If it still says **v360** or older, you are on the old build. Do not open a caller on v360.

## There is a service worker

`offgrd-sw.js` caches the gameday shell so airplane mode still boots.
Cache name is `offgrd-gameday-v361`. A pin changes that name. On activate the
...
4. Read the chip: it must say **v361**.
