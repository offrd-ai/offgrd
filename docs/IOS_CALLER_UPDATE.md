...
The caller chip **v339** (top bar, next to Snap N) is the only proof the refresh took.
If it still says **v338**, you are on the old build. Do not open a caller on v338.

## There is a service worker

`offgrd-sw.js` caches the gameday shell so airplane mode still boots.
Cache name is `offgrd-gameday-v339`. A pin changes that name. On activate the
...
4. Read the chip: it must say **v339**.
