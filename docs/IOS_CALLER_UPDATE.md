...
The caller chip **v352** (top bar, next to Snap N) is the only proof the refresh took.
If it still says **v351** or older, you are on the old build. Do not open a caller on v351.

## There is a service worker

`offgrd-sw.js` caches the gameday shell so airplane mode still boots.
Cache name is `offgrd-gameday-v352`. A pin changes that name. On activate the
...
4. Read the chip: it must say **v352**.
5. If it still says v351, stay in that app, pull down to refresh once, force-quit, reopen. Do not go to Settings.
