...
The caller chip **v338** (top bar, next to Snap N) is the only proof the refresh took.
If it still says **v337**, you are on the old build.

## There is a service worker

`offgrd-sw.js` caches the gameday shell so airplane mode still boots.
Cache name is `offgrd-gameday-v338`. A pin changes that name. On activate the
...
4. Read the chip: it must say **v338**.
