...
Production is **v363** (gameday picker + pin). The caller chip is the only
proof. If it still says **v360**, force-quit on wifi and reopen so the
network-first SW can pick up v363.

## There is a service worker

`offgrd-sw.js` caches the gameday shell so airplane mode still boots.
Cache name is `offgrd-gameday-v363`. A pin changes that name. On activate the
...
4. Read the chip: it must say **v363**.
