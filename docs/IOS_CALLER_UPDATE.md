...
The caller chip **v347** (top bar, next to Snap N) is the only proof the refresh took.
If it still says **v346** or older, you are on the old build. Do not open a caller on v346.

## There is a service worker

`offgrd-sw.js` caches the gameday shell so airplane mode still boots.
Cache name is `offgrd-gameday-v347`. A pin changes that name. On activate the
...
4. Read the chip: it must say **v347**.
5. If it still says v346, stay in that app, pull down to refresh once, force-quit, reopen. Do not go to Settings.
